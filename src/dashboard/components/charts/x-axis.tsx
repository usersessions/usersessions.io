...
export interface XAxisProps {
  ...
  numTicks?: number;
  /**
   * Explicit tick positions — every tick lands exactly here and nowhere
   * else, bypassing data/domain tick building. The way to keep a clock axis
   * on round hours (00, 03, 06 …) instead of interpolated times.
   */
  tickDates?: Date[];
...
interface XAxisLabelProps {
  ...
  /** Plot width — labels clamp inside `[edgePad, width - edgePad]`. */
  clampWidth: number;
  /** Inset kept between a clamped label's center and the plot edge. */
  edgePad: number;
}
...
function XAxisLabel({
  ...
  // Keep the first/last labels inside the plot even when the chart itself is
  // flush with the card edges (zero side margins).
  const clampedX =
    clampWidth > edgePad * 2
      ? Math.min(Math.max(x, edgePad), clampWidth - edgePad)
      : x;
  const fadeBuffer = 20;
...
function appendProjectionTailTicks(
  ...
  const endLabel = formatLabel(domainEnd);
...
const XAxisInner = memo(function XAxisInner({
  ...
  const labelsToShow = useMemo(() => {
    const formatLabel = xLabelFormat ?? shortDateFmt.format;

    if (tickDates && tickDates.length > 0) {
  ...
  const hoveredLabel =
    isHovering && tooltipData
      ? (dateLabels[tooltipData.index] ??
        (xLabelFormat ?? shortDateFmt.format)(xAccessor(tooltipData.point)))
      : null;

  // Full plot width the labels are positioned within; used to clamp the
  // first/last label inside the plot when the chart runs edge-to-edge. The pad
  // is the full ticker half-width so a clamped label sits entirely inside.
  const clampWidth = container.clientWidth;
  const edgePad = tickerHalfWidth;
